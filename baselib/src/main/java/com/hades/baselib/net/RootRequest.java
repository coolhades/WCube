        ...
        public void onResponse(Call<T> call, Response<T> response) {
            ...
            if(response.isSuccessful()) {
                if(((RootDataBean)response.body()).Status ==1) {
                    Log.d("TAG-Success", gson.toJson((T) ((RootDataBean) response.body()).BizData));
                    mCallback.onSuccess( (T) ((RootDataBean) response.body()).BizData );
                } else {
                    Log.d("TAG-Error", "出错:"+((RootDataBean)response.body()).Msg);
                    mCallback.onError(((RootDataBean)response.body()).Msg);
                }
            } else {
                mCallback.onFailure();
            }
